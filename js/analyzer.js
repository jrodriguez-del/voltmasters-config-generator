...
// ═══════════════════════════════════════════
// ANÁLISIS 5: IMPORT LIMIT (desde curvas)
// Analiza en qué nivel de importación la batería
// realmente descarga y qué pico mantiene.
// ═══════════════════════════════════════════

export function analyzeImportLimit(rows, batteryConfig) {
  if (!rows || rows.length === 0) return null;

  const socMin = (batteryConfig?.soc_min_pct || 10) / 100 * (batteryConfig?.capacidad_kwh || 0);
  const socThreshold = Math.max(socMin * 1.1, 43); // slightly above minimum SoC

  // 1. Find peak shaving events: battery discharging with SoC available
  const psEvents = [];
  for (const r of rows) {
    const descarga = r.BESS_Descarga_Neta_kW || 0;
    const picoSQ = r.Pico_Red_SQ_kW || 0;
    const picoBESS = r.Pico_Red_BESS_kW || 0;
    const soc = r.BESS_SoC_kWh || 0;

    if (descarga > 5 && picoSQ > 500) {
      psEvents.push({ picoSQ, picoBESS, descarga, soc, effective: soc > socThreshold });
    }
  }

  if (psEvents.length < 10) return null; // Not enough data

  // 2. Effective events (battery had energy to discharge)
  const effectiveEvents = psEvents.filter(e => e.effective);
  const effLevels = effectiveEvents.map(e => e.picoBESS).sort((a, b) => a - b);
  ...
  for (const r of rows) {
    ...
    const picoSQ = r.Pico_Red_SQ_kW || 0;
    const picoBESS = r.Pico_Red_BESS_kW || 0;
    if (!dailyPeaks[day] || picoSQ > dailyPeaks[day].sq) {
      dailyPeaks[day] = { sq: picoSQ, bess: picoBESS };
    }
  }
  const dailySQ = Object.values(dailyPeaks).map(d => d.sq).sort((a, b) => a - b);
  const dailyBESS = Object.values(dailyPeaks).map(d => d.bess).sort((a, b) => a - b);

  // 4. Monthly peaks
  const monthlyPeaks = {};
  for (const r of rows) {
    const m = String(r.Fecha).substring(3, 10);
    const picoSQ = r.Pico_Red_SQ_kW || 0;
    const picoBESS = r.Pico_Red_BESS_kW || 0;
    if (!monthlyPeaks[m]) monthlyPeaks[m] = { sq: 0, bess: 0 };
    if (picoSQ > monthlyPeaks[m].sq) monthlyPeaks[m].sq = picoSQ;
    if (picoBESS > monthlyPeaks[m].bess) monthlyPeaks[m].bess = picoBESS;
  }

  const pctl = (arr, p) => arr[Math.floor(arr.length * p)] || 0;

  // 5. Recommendation: P90/P95 of the effective maintained peak
  const p90 = effLevels.length > 0 ? pctl(effLevels, 0.90) : 0;
  const p95 = effLevels.length > 0 ? pctl(effLevels, 0.95) : 0;
  const recommendation = Math.ceil(p95); // P95 = what the battery maintains 95% of the time

  return {
    totalEvents: psEvents.length,
    effectiveEvents: effectiveEvents.length,
    // Effective peak maintained during discharge (battery had energy)
    effective_p50: effLevels.length > 0 ? pctl(effLevels, 0.50) : 0,
    effective_p75: effLevels.length > 0 ? pctl(effLevels, 0.75) : 0,
    effective_p90: Math.ceil(p90),
    effective_p95: Math.ceil(p95),
    effective_max: effLevels.length > 0 ? effLevels[effLevels.length - 1] : 0,
    // Daily peaks
    daily_sq_p90: pctl(dailySQ, 0.90),
    daily_sq_p95: pctl(dailySQ, 0.95),
    daily_bess_p90: pctl(dailyBESS, 0.90),
    daily_bess_p95: pctl(dailyBESS, 0.95),
    // Monthly (for renderer)
    monthlyPeaks,
